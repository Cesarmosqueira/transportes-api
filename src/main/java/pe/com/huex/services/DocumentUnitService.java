...
import pe.com.huex.repositories.DocumentsUnitRepository;
...
public class DocumentUnitService {
    ...
    @Autowired
    DocumentsUnitRepository documentsUnitRepository;

    public ResponseDto<DocumentUnitListDto> listDocumentUnits() {
        ResponseDto<DocumentUnitListDto> response = new ResponseDto<>();
        try {
            ...
            response.setDatos(new DocumentUnitListDto().documentUnitList(documentUnitList));
    ...
    public ResponseDto<DocumentUnitRetrieveDto> retrieveDocumentUnit(Long id) {
        ResponseDto<DocumentUnitRetrieveDto> response = new ResponseDto<>();
        try {
            String idTransaccion = UUID.randomUUID().toString();
            ...
            response.meta(MetaDatosUtil.buildMetadatos(CODE_SUCCESS, MESSAGE_RETRIEVE_DOCUMENTUNIT_SUCCESS, INFO, idTransaccion)
                    .totalRegistros(1));
            response.setDatos(new DocumentUnitRetrieveDto().documentUnit(documentUnitList.get()));
    ...
    public ResponseDto<DocumentUnitRegisterDto> registerDocumentUnit(DocumentUnit documentUnit) {
        ResponseDto<DocumentUnitRegisterDto> response = new ResponseDto<>();

        try {
            String idTransaccion = UUID.randomUUID().toString();
            DocumentUnit DocumentUnitResponse = documentsUnitRepository.save(documentUnit);
            response.meta(MetaDatosUtil.buildMetadatos(CODE_SUCCESS, MESSAGE_REGISTER_DOCUMENTUNIT_SUCCESS, INFO, idTransaccion));
            response.setDatos(new DocumentUnitRegisterDto().documentUnit(DocumentUnitResponse));
        } catch (Exception ex) {
    ...
    public ResponseDto<DocumentUnitUpdateDto> updateDocumentUnit(Long id, DocumentUnit documentUnit) {
        ResponseDto<DocumentUnitUpdateDto> response = new ResponseDto<>();

        try {
            String idTransaccion = UUID.randomUUID().toString();

            DocumentUnit documentUnitResponse = documentsUnitRepository.findById(id).get();

            if (Objects.isNull(documentUnitResponse)) {
                response.meta(MetaDatosUtil.buildMetadatos(CODE_WARN, MESSAGE_RETRIEVE_DOCUMENTUNIT_WARN, WARN, idTransaccion)
                        .totalRegistros(0));
                return response;
            }
            ...
            documentsUnitRepository.save(documentUnit);
            response.meta(MetaDatosUtil.buildMetadatos(CODE_SUCCESS, MESSAGE_UPDATE_DOCUMENTUNIT_SUCCESS, INFO, idTransaccion));
            response.setDatos(new DocumentUnitUpdateDto().documentUnit(documentUnit));
